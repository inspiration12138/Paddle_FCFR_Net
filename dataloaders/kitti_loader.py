...
def load_calib():
    ...
    从calib_cam_to_cam中获得相机的内参矩阵K，并返回K（3*3）
    """
...
def get_paths_and_transform(split,args):
    '''
    :param split: "train" or "val" or "test_completion" or "test_prediction"
    :param args: 主要看arg.val
    :return: paths（见下）, transform对应的图片预处理操作
    ...
    if split == "train":
        ...
        if args.val == "full":
            ...
            glob_d = os.path.join(
                ...
                'data_depth_dense_new/val/*_sync/proj_depth/groundtruth/image_0[2,3]/*.png')
            # )
            glob_gt = os.path.join(
                args.data_folder,
        ...
        glob_d = os.path.join(
            args.data_folder,
            "depth_selection/test_depth_completion_anonymous/data_depth_dense_new/*.png"
        )
...
oheight, owidth = 352, 1216 # raw 1224 * 370


def drop_depth_measurements(depth, prob_keep):
    '''以prob_keep概率做1次伯努利二项分布实验，将depth中的数值随机遮盖为0，返回遮盖后的depth'''
    mask = np.random.binomial(1, prob_keep, depth.shape) #一共做1次实验，以prob_keep概率返回1,形状与depth相同
    depth *= mask
    print(mask)
    return depth
...
def train_transform(rgb, sparse, target, rgb_near, args):
    '''
    对rgb，rgb_near做颜色和几何上的预处理，对sparse和target做几何上的预处理
    '''
    # s = np.random.uniform(1.0, 1.5) # random scaling
    # angle = np.random.uniform(-5.0, 5.0) # random rotation degrees
    do_flip = np.random.uniform(0.0, 1.0) < 0.5  # random horizontal flip 从0-1均匀分布中随机采样
...
to_float_tensor = lambda x: to_tensor(x).astype(paddle.float32)


def handle_gray(rgb, args):
    '''将rgb转为gray，返回 rgb(if args.use_rgb), img(gray)'''
    if rgb is None:
        return None, None
    if not args.use_g:
        return rgb, None
    else:
        img = np.array(Image.fromarray(rgb).convert('L')) # 转为灰度图像
...
def get_rgb_near(path, args):
    ...
    while True:
        random_offset = choice(candidates) # 随机选择前后3张中的一张
        path_near = get_nearby_filename(path, number + random_offset) # 获得选中的这一张的filename
...
class KittiDepth(data.Dataset):
    ...
    return：{"rgb":rgb, "d":sparse, "gt":target, "g":gray, "r_mat":r_mat, "t_vec":t_vec, "rgb_near":rgb_near}
    """
...
#             path_part['gt'] = []
#             for p in part:
#                 path_part['rgb'].append(self.paths['rgb'][p])
#                 path_part['d'].append(self.paths['d'][p])
#                 path_part['gt'].append(self.paths['gt'][p])
#             self.paths = path_part
