...
def _is_numpy_image(img):
    '''判断是否为numpy数组，维度是否为2/3维，同时满足返回True'''
...
def adjust_saturation(img, saturation_factor):
    """Adjust color saturation of an image.饱和度
...
def adjust_hue(img, hue_factor):
    """Adjust hue of an image在HSV空间改色度.
...
class ToTensor(object):
    ...
    def __call__(self, img):
        ...
        if isinstance(img, np.ndarray):
            # handle numpy array
            if img.ndim == 3:
                img = paddle.to_tensor(img.transpose((2, 0, 1)).copy())
            elif img.ndim == 2:
                img = paddle.to_tensor(img.copy())
            else:
                raise RuntimeError(
