...
class Result(object):
    ...
    def evaluate(self, output, target, depth_output, lidar_output, photometric=0):
        ...
        abs_diff = paddle.abs(output_mm - target_mm)
        ...
        maxRatio = paddle.maximum(output_mm / target_mm, target_mm / output_mm)
        self.delta1 = float(paddle.mean((maxRatio < 1.25).astype(paddle.float32)))
        self.delta2 = float(paddle.mean((maxRatio < 1.25**2).astype(paddle.float32)))
        self.delta3 = float(paddle.mean((maxRatio < 1.25**3).astype(paddle.float32)))
        self.data_time = 0
        self.gpu_time = 0
    ...
    def average(self):
        ...
        if self.count > 0:
            avg.update(
                ...
                self.sum_irmse1 / self.count, self.sum_imae1 / self.count,
                self.sum_mse1 / self.count, self.sum_rmse1 / self.count,
                self.sum_mae1 / self.count,
                self.sum_irmse2 / self.count, self.sum_imae2 / self.count,
                self.sum_mse2 / self.count, self.sum_rmse2 / self.count,
                self.sum_mae2 / self.count,
