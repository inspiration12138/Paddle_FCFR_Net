...
def feature_fuse(d1, d2):
    ...
    d1_2 = d1.multiply(d1)
    d2_2 = d2.multiply(d2)
    E1 = my_conv(d1_2.reshape(shape=[-1, 1, h, w]))
    E2 = my_conv(d2_2.reshape(shape=[-1, 1, h, w]))
    E1 = E1.reshape(shape=[b, c, h, w])
    E2 = E2.reshape(shape=[b, c, h, w])

    ones = paddle.ones_like(d1)
    zeros = paddle.zeros_like(d1)

    mask1 = paddle.where(E1 < E2, zeros, ones)
    F11 = d1 * mask1

    mask2 = paddle.where(E1 >= E2, zeros, ones)
    F12 = d2 * mask2
...
def conv_bn_relu(in_channels, out_channels, kernel_size, stride=1,
                 padding=0, bn=True, relu=True):
    bias = not bn
    layers = []
    layers.append(
